...
- **Phase 8 (Branch Prediction & CPI Experiments):** Complete in Simulation. CPI=1.258 measured on branch_sort.mem with 64-entry BHT (20,613 cycles, 16,387 instructions, 65 flushes per iteration). Results saved to results/phase8_cpi_metrics.txt. Baseline not directly measured — BHT has no disable/bypass path without RTL modification, which is out of scope.
