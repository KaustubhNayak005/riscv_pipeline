...
This file is generated. Run `tools/update_docs.ps1` after architecture, roadmap, HDL, simulation, diagram, or Vivado report changes.

The generator reads HDL sources, generated program files, generated diagram sources, simulation logs, Vivado reports, and `userdocs/roadmap.md`. It is intentionally conservative: if it cannot prove an architecture feature from project files, it reports that feature as not detected.

---

## Current Architecture Summary

The implemented system is a small bare-metal RISC-V soft SoC centered on a 5-stage RV32I pipelined processor. The current verified architecture includes: a ROM-preloaded instruction memory with a UART monitor-driven loader, data RAM, UART MMIO, performance-counter MMIO, debug MMIO, a 4-entry commit trace buffer, forwarding, load-use stall handling, branch/jump flushing, subword memory operations, FENCE/FENCE.I as NOP, ECALL/EBREAK halt behavior, and illegal instruction detection.
...
| Timing | PASS: WNS +5.556 ns, all user timing constraints met (2025.2 build) |
...
Run `tools/update_docs.ps1` after any implementation work that changes HDL, generated programs, simulation results, Vivado reports, roadmap phases, or architecture-visible behavior.
